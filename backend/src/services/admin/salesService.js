...
import { getAllStatuses } from '../../helpers/orderStatus.js';
...
function isPrismaTlsError(error) {
  ...
  return error?.errorCode === 'P1011' || message.includes('Error opening a TLS connection');
}
...
async function getSalesDataViaPg(req) {
...
async function searchProductsForOrderEditViaPg(req) {
...
export const getSalesData = async (req, res) => {
  try {
    ...
    if (isPrismaTlsError(error)) {
      try {
        const fallback = await getSalesDataViaPg(req);
        return res.json(fallback);
      } catch (fallbackError) {
        return erro(res, fallbackError.message, 500);
      }
    }
...
export const searchProductsForOrderEdit = async (req, res) => {
  try {
    ...
    if (isPrismaTlsError(error)) {
      try {
        const fallback = await searchProductsForOrderEditViaPg(req);
        return res.json(fallback);
      } catch (fallbackError) {
        return erro(res, fallbackError.message, 500);
      }
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(orderId)) {
      return erro(res, 'ID de pedido inválido.', 400);
    }
    ...
    if (currentOrder.status === status) {
      return sucesso(res, { mensagem: 'Status já está atualizado', pedido: currentOrder });
    }
...
export const bulkUpdateOrderStatus = async (req, res) => {
  try {
    ...
    return sucesso(res, { mensagem: `${updated} pedidos atualizados, ${skipped} ignorados (já no status)`, updated, skipped });
  } catch (error) {
    return erro(res, error.message, 500);
  }
};
